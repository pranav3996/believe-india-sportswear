'use client';

import Image from 'next/image';

export default function TestimonialCard({ testimonial }) {
    const name = testimonial.name || testimonial.customerName || 'Anonymous';
    const message = testimonial.message || testimonial.text || '';
    const email = testimonial.email;
    const company = testimonial.company;
    const rating = testimonial.rating || 5;
    const avatarUrl = testimonial.avatarUrl || testimonial.image;
    const createdAt = testimonial.createdAt;

    // Generate initials for fallback avatar
    const getInitials = (name) => {
        if (!name || name === 'Anonymous') return 'A';
        return name
            .split(' ')
            .map(word => word[0])
            .join('')
            .toUpperCase()
            .slice(0, 2);
    };

    // Format date
    const formatDate = (dateString) => {
        if (!dateString) return '';
        const date = new Date(dateString);
        return date.toLocaleDateString('en-US', {
            month: 'short',
            year: 'numeric'
        });
    };

    // Render star rating
    const renderStars = (rating) => {
        return [...Array(5)].map((_, index) => (
            <svg
                key={index}
                className={`w-5 h-5 ${index < rating ? 'text-yellow-400' : 'text-gray-300'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
            >
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
        ));
    };

    return (
        <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
            {/* Rating */}
            <div className="flex items-center mb-4">
                {renderStars(rating)}
            </div>

            {/* Quote Icon */}
            <div className="mb-4">
                <svg className="w-10 h-10 text-primary-200" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
                </svg>
            </div>

            {/* Message */}
            <p className="text-gray-700 mb-6 italic flex-grow leading-relaxed">
                {message}
            </p>

            {/* Author Section */}
            <div className="border-t pt-4 mt-auto">
                <div className="flex items-center">
                    {/* Avatar */}
                    <div className="mr-4 flex-shrink-0">
                        {avatarUrl ? (
                            <Image
                                src={avatarUrl}
                                alt={name}
                                width={48}
                                height={48}
                                className="rounded-full object-cover border-2 border-primary-200"
                            />
                        ) : (
                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary-400 to-accent-500 flex items-center justify-center text-white font-bold">
                                {getInitials(name)}
                            </div>
                        )}
                    </div>

                    {/* Name and Details */}
                    <div className="flex-grow">
                        <p className="font-semibold text-gray-900 text-lg">
                            {name}
                        </p>
                        {company && (
                            <p className="text-sm text-gray-600">{company}</p>
                        )}
                        {!company && email && (
                            <p className="text-sm text-gray-600">{email}</p>
                        )}
                        {createdAt && (
                            <p className="text-xs text-gray-500 mt-1">
                                {formatDate(createdAt)}
                            </p>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}
